...
cppcoro::task<> setCommonCurlOptions(CURL* curl, const std::string& user_agent, const std::string& url,
...
cppcoro::task<Json::Value> LoggedInClient::get(const std::string& url) const
{
    ...
    std::string str_buffer;
    co_await setCommonCurlOptions(curl, user_agent, url, str_buffer);
